...
public class Sorting {
	...
	private static void RecursiveMergeSort(int[] A) { // Ex3 Complete method RecursiveMergeSort
		...
		int[] leftHalf = new int[midIndex]; //declare index
		int[] rightHalf = new int[inputLength - midIndex]; // declare index

		for (int i = 0; i < midIndex; i++) {
			leftHalf[i] = A[i]; // assign value
		}
		for (int i = midIndex; i < inputLength; i++) {
			rightHalf[i - midIndex] = A[i]; // assign value
		}
	...
	static void RecursiveQuickSort(int[] A, int start, int end) { // Ex4. Complete the method RecursiveQuickSort
		if (start < end) {
			...
			RecursiveQuickSort(A, start, end);
		}
